...
/// Initial container/codec workloads. No tiled FITS or codec-internal parallelism.
...
/// Version-one recipe for UInt16 monochrome frames. All bounds are checked before use.
...
impl Recipe {
    ...
    pub fn validate(&self) -> Result<u64> {
        ...
        let maximum = (decoded + decoded / 100 + 16384) * self.frames as u64 + MANIFEST_LIMIT;
        ensure!(
...
pub struct Manifest {
    ...
    /// Pixel/container recipe implementation version, currently 1.
...
impl FixtureSet {
    ...
    pub fn open(directory: &Path, cancel: &AtomicBool) -> Result<Self> {
        ...
        let manifest: Manifest = serde_json::from_slice(&bytes)?;
            ...
            manifest.schema_version == 1 && manifest.generator_version == 1,
...
fn pixels(
    ...
    let count = u64::from(recipe.width) * u64::from(recipe.height);
    let mut state = recipe
        .seed
    ...
    while offset < count {
        checkpoint(cancel)?;
        let n = (count - offset).min((CHUNK / 2) as u64) as usize;
        for i in 0..n {
            let value = match recipe.pattern {
                Pattern::Noise => {
                    state ^= state << 13;
                    state ^= state >> 7;
                    state ^= state << 17;
                ...
                Pattern::Gradient => {
                    (((offset + i as u64) % u64::from(recipe.width)) * 65535
                        / u64::from(recipe.width.max(2) - 1)) as u16
                }
            };
            // FITS stores signed samples with BZERO=32768; preserve the same
            // logical UInt16 pixel values as the XISF little-endian payload.
            buffer[2 * i..2 * i + 2].copy_from_slice(&if recipe.encoding == Encoding::Fits {
                (value ^ 0x8000).to_be_bytes()
            } else {
                value.to_le_bytes()
            });
        }
        writer.write_all(&buffer[..n * 2])?;
        offset += n as u64;
    }
    Ok(())
}
